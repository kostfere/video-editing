...
def start_processing(videos, target_fps, progress_bar):
    ...
    thread = Thread(
        ...
        args=(
            videos,
            target_fps,
            lambda n: update_progress_bar(n, total_videos, progress_bar),
        ),
        daemon=True,
    )
    thread.start()
...
def select_videos():
    file_paths = filedialog.askopenfilenames(
        title="Select Videos", filetypes=(("MP4 files", "*.mp4"), ("All files", "*.*"))
    )
    if not file_paths:
        return
    target_fps = simpledialog.askinteger(
        "Input", "Enter the desired FPS:", minvalue=1, maxvalue=60
    )
...
select_button = tk.Button(
    frame, text="Select Videos and Set FPS", command=select_videos
)
