...
class VideoTrimmerApp:
    ...
    def setup_ui(self):
        Label(
            self.parent, text="Video Trimmer (Single Video)", font=("Arial", 16)
        ).pack(pady=20)

        Button(self.parent, text="Select Video", command=self.select_video).pack(pady=5)
        self.video_label = Label(
            self.parent, text="No video selected", font=("Arial", 10)
        )
        ...
        self.save_button = Button(
            ...
            command=self.save_trimmed_video,
            state="disabled",
        )
    ...
    def select_video(self):
        ...
        if file_path:
            ...
            self.save_button["state"] = (
                "disabled"  # Initially disable save until a preview has been made
            )
    ...
    def preview_trim(self):
        if (
            not self.video_path
            or not self.start_timestamp_entry.get()
            or not self.end_timestamp_entry.get()
        ):
            messagebox.showerror(
                "Error",
                "Please select a video and specify both start and end timestamps.",
            )
            return
        try:
